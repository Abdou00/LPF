<style>
	...
	h2 { color: orange; border-top: 1px solid gray; border-bottom: 1px solid gray; }	
</style>
...
// Remarque sur echo :
echo $x, $y, '<br>';  // dans l'instruction echo on peut séparer les éléments à afficher par une virgule. Cette syntaxe est spécifique eu echo, et ne marche pas avec print.
...
//----------
// Exercice : réécrire le switch précédent en conditions if... classiques. On doit obtenir le même résultat.
...
echo strlen($phrase); // strlen retourne la taille d'une chaine (nombre d'octets de cette chaîne, un caractère accentué valant 2 octets).
...
function bonjour($qui) {  // $qui apparaît ici pour la première fois : il permet de recevoir un argument. Il s'agit d'une variable de réception. Notez que l'on peut mettre plusieurs paramètres dans les parenthèses séparés par une virgule
...
function appliqueTva2($nombre, $taux = 1.2) {  // on peut initialiser par défaut un paramètre dans le cas où on ne reçoit de valeur en argument lors de l'appel de la fonction. On a renommé notre fonction car on ne peut pas déclarer 2 fonctions qui portent le même nom
	return $nombre * $taux;
}
...
//---------
// Exercice : à l'aide d'une boucle while, afficher dans un sélecteur les années de 1918 à 2018. 
echo '<select>';
...
echo '</table>';  // nous avons ici le principe des boucles imbriquées. Quand la 1ere boucle fait 1 tour, la boucle intérieure en fait 10.
...
//--------------------
// Un tableau, ou array en anglais, est déclarée comme une variable améliorée dans laquelle on stocke une multitude de valeurs. Ces valeurs peuvent être de n'importe quel type. Elles possèdent un indice dont la numérotation par défaut commence à 0.
...
// Exercice : écrivez un array associatif avec les indices prenom, nom, email et telephone, et mettez y des informations pour une seule personne. Puis avec une boucle foreach, affichez les valeurs dans des <p>, sauf pour le prenom qui doit être dans un <h3>.
...
foreach ($tab_multi as $sous_tableau) {
	// debug($sous_tableau);
	echo $sous_tableau['prenom'] . '<br>';
}

echo '<hr>';
// Exercice : afficher TOUTES les valeurs de l'array $tab_multi en utilisant foreach.
foreach ($tab_multi as $sous_tableau) {
	foreach ($sous_tableau as $info) { // $sous_tableau étant lui même un array, on le parcourt aussi avec une foreach
...
//--------------------
echo date('d/m/Y H:i:s') . '<br>';  // date() retourne la date de maintenant selon le format indiqué. d pour jour, m pour mois, Y pour année sur 4 chiffres, y pour année sur 2 chiffres, H pour heure sur 24h, h pour heure sur 12h, i pour minute et s pour seconde.   

echo date('Y-m-d') . '<br>';  // on peut changer l'ordre des paramètres ainsi que le séparateur
...
//--------------------
// Un objet est un autre type de données. Il représente un objet du réel (par exemple : une voiture, un meuble, un personnage, etc...) auquel on peut associer des caractéristiques appelées propriétés (ou attributs), ainsi que des fonctions pour faire des actions appelées méthodes.
...
}  // cette classe est un "plan de construction" d'objet "meuble" qui pourront utiliser la propriété $marque et la méthode prix()


// Puis nous créons une table à partir de cette classe :

$table = new Meuble(); // new est mot clé qui permet d'instancier la classe Meuble pour en faire un objet $table. On dit que $table est une instance de Meuble.
...
echo 'Le prix de ma table est : ' . $table->prix() . ' euros <br>'; // pour accéder à une méthode d'un objet, on écrit l'objet suivi d'une flèche "->" suivi du nom de la méthode avec une paire de ()
